...
int main(int argC, char **argV, char **envP) {
    ...
    //Main loop.
    __pid_t processID; //__pid_t is a special data type to work with processes.
    int status; //Required to pass into wait, unsure of the reason yet...
    //while(scanf("%s", line) != EOF) {
    while(fgets(line, COMMAND_LIMIT, stdin) != NULL) {
        /* Use fork, execv, and wait*/
